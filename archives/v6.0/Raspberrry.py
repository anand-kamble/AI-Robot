...
def imagePro():
    ...
    #camera.capture(camStream, format="jpeg")
    #encodedImage = base64.b64encode(camStream.getvalue())
...
def startStreamingVideo():
    for frame in camera.capture_continuous(camStream, format="jpeg", use_video_port=True):
        encodedImage = base64.b64encode(frame.getvalue())
        sio.emit('ReceiveStreamFrame',encodedImage)

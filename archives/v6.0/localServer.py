...
def processImage(imageString):
    ...
    #print(imageString)
    if len(lastImageString) > 0:
            imgBuffer = base64.b64decode(lastImageString);
            #NumArray = np.frombuffer(imgBuffer, dtype=np.uint8)#np.fromstring(imageString, dtype=np.uint8).reshape(720,1280,3)
